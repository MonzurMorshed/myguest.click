...
class Menu extends Controller {
    ...
    public function index() {
        ...
        /* Get the available healthservices of store */
        $health_services = db()->where('store_id', $this->store->store_id)->get('store_health_services');
        

        /* Set a custom title */
        if($this->store->settings->title) {
            Title::set(sprintf(l('s_menu.title'), $this->menu->name, $this->store->settings->title), true);
        } else {
            Title::set(sprintf(l('s_menu.title'), $this->menu->name, $this->store->name));
        }

        /* Set the meta tags */
        Meta::set_description(string_truncate($this->menu->description, 160));
        ...
        Meta::set_social_description(string_truncate($this->menu->description, 160));
        Meta::set_social_image($this->menu->image ? \Altum\Uploads::get_full_url('menu_images') . $this->menu->image : null);
        Meta::set_canonical_url($this->store->full_url . $this->menu->url);
