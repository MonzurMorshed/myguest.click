...
class Database {
    ...
    public static function initialize() {
        ...
        if(self::$database->connect_error) {
            ...
            die('The connection to the database failed! Check the config.php file and make sure your database connection details are correct and your server is running. - '.$mysqli -> connect_error);
        }
    ...
    public static function get($what, $from, Array $conditions = [], $order = false, $clean = true) {
        ...
        $result = database()->query("SELECT {$what} FROM {$from} WHERE {$where} {$order_by}");
